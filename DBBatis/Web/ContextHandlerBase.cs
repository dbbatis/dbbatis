    ...
    public abstract class ContextHandlerBase
    {
        ...
        public Task Precess()
        {
            ...
            if (!MainConfig.IsEncryption)
            {
                ...
                else if (endname.EndsWith("cnn"))
                {
                    ...
                    {
                        string key = this.Data["DBFlag"];
                        if (string.IsNullOrEmpty(key))
                        {
                            cnn = MainConfig.GetDbConfig().ConnectionString;
                        }
                        else
                        {
                ...
                {
                    return Write(Url.ToJson());
                    //return Write(string.Format("{0}:{1}", Url.Host, Url.Port));
        ...
        private Task WriteActionList(Page pc)
        {
            ...
            if (pc.DbActions != null && pc.DbActions.Count > 0)
            {
                ...
                {
                    sb.AppendFormat("<li><a href=\"{0}&ActionName={1}\">【{1}】</a>{2}</li>", Url.OriginalString, actionName, pc.DbActions[actionName].Description);
                }
                sb.Append("</ol>");
            ...
            if (pc.BatchDbActions != null && pc.BatchDbActions.Count > 0)
            {
                ...
                foreach (string actionName in pc.BatchDbActions.Keys)
                {
                    ...
                    sb.AppendFormat("<li><a href=\"{0}&ActionName={1}\">【{1}】</a>{2}</li>", Url.OriginalString, actionName, pc.BatchDbActions[actionName].Description);
                }
                sb.Append("</ol>");
...
,Data:", pageid, actionname));
                        ...
                        for (int i = 0; i < actionCommand.Command.Parameters.Count; i++)
                        {
                            ...
                            if (i != 0)
                                sbdata.Append(",");
                            sbdata.AppendLine(string.Format("           {0}:''   //{1}", p.SourceColumn, desc));
                        }
                        sbdata.AppendLine("         }");
